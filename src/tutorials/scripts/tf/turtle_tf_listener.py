...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        try:
            # listener 同时订阅 turtle2 和 turtle1 的 tf 坐标,然后计算得到 2 对于 1 的相对位置
            # 并发布 turtle2 对 turtle1 的运动向量
            # ---------------------------------------------------------------
            # 查找 '/turtle2' 和 '/turtle1' 坐标系之间的变换
            # listener.lookupTransform返回两个值：平移（trans）和旋转（rot）
            # 这里我们查询的是 turtel2 相对于 turtle1 的坐标变换
        ...
        # trans[0] 是 turtle2 到 turtle1 的 x 方向的位移
        # trans[1] 是 turtle2 到 turtle1 的 y 方向的位移
        # math.atan2(trans[1], trans[0]) 计算的是 turtle2 需要转动的角度
        #  - turtle2 当前位置到 turtle1 之间的直线的角度
        # 4 是一个系数，用来 放大 计算出来的角度。这意味着海龟调整方向的速度会更快
        angular = 4 * math.atan2(trans[1], trans[0])
        # 计算线速度：math.sqrt(trans[0] ** 2 + trans[1] ** 2) 计算的是 turtle2 到 turtle1 的 欧几里得距离
        # 0.5 * 距离,得到线速度
